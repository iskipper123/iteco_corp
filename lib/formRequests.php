<?php
	...
	if(isset($_POST["editMarkedItem"])) {
		...
		if(!isset($idItem))
			...
			header("Location: editformRequest.php?edit=$idItem");
			exit;
		}
	} 
	if(isset($_POST["addToRequest"])) { 
		...
		if(!isset($idItem))
			...
			$result_set = $db->getRowWhere("form_requests_customer", "id", $idItem);
			...
			if($row[status] == 1) {
				...
				header("Location: formRequestToRequest.php?edit=$idItem");
				exit; 
			}
		}
	}
	...
	if(isset($_POST["comfirmDelete"])) {
		...
		$db->delete("form_requests_customer", $idItem);
	...
	if(isset($_POST["filter"])) {
		...
		if(strlen($month) == 0 || strlen($year) == 0) {
			...
			if($_SESSION["role"] == 1) 		$result_set = $db->formRequestsByMonth($startDate, $amountOfDaysInMonth);
			else if($_SESSION["role"] == 2) $result_set = $db->formRequestsByMonthByManager($startDate, $amountOfDaysInMonth, $_SESSION["id"]);
		}
	}
		...
		else $result_set = $db->formRequestsByMonthByManager($idUser);
	}
	else {
		$today = date("Y-m-d");
		...
		if($_SESSION["role"] == 1) 		$result_set = $db->formRequestsByMonth($startDayForCurrentMonth, $amountOfDaysInMonth);
		else if($_SESSION["role"] == 2)	$result_set = $db->formRequestsByMonthByManager($startDayForCurrentMonth, $amountOfDaysInMonth, $_SESSION["id"]);
	}
    ...
    <div class="col-md-12">
        <? if($addConfirmButton) {?>
            <form name="" action="" method="post">
                <div class="card col-md-5">
                    <div class="card-body">
                        ...
                        <?
                            $result_set4 = $db->getRowWhere("form_requests_customer", "id", $idItem);
            ...
            <hr>
        <?}?><!-- 
        <a class="btn btn-success btn-sm" href="/formRequestCustomer.php" target="_blank"><i class="fas fa-plus"></i> Добавить заявку</a> <br /><br /> -->
<!--         <hr style="clear: both;">
<form name="" action="" method="post">
    <div class="row">
        ...
        <? if($_SESSION["role"] == 1) { ?>
            <form name="" action="" method="post" class="check_user">
                <div class="row">
                    <div class="form-group col-md-2">
        ...
        <? if($showTable) require_once "../lib/allFormRequests.php"; ?>
    </div>
