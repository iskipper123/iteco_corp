...
<?php
	...
	if(isset($_POST["filter"])) {
		...
		if(strlen($month) == 0 || strlen($year) == 0) {
			...
			$result_set = $db->getCustomsClearanceByMonth($startDate, $amountOfDaysInMonth);
		}
	}
	else {
		// $result_set = $db->getAllOrder("customs_clearance", "date DESC");
		...
		$result_set = $db->getCustomsClearanceByMonth($startDayForCurrentMonth, $amountOfDaysInMonth);
	}
