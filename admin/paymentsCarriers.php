...
<h1>Договоры Перевозчики</h1>
<?php
	
    $title = 'Оплаты перевозчикам';
	$showTable = true;
	...
	if(isset($_POST["filter"])) {
		...
		if(strlen($month) == 0 || strlen($year) == 0) {
			...
			$result_set = $db->getPaymentsByMonth($startDate, $amountOfDaysInMonth, $arrayOfContractorsTypes[1]);
		}
	}
		...
		$result_set = $db->getPaymentsByMonth($startDayForCurrentMonth, $amountOfDaysInMonth, $arrayOfContractorsTypes[1]);
	}
