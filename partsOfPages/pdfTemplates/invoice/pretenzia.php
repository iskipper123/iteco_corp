...
<html lang="ru">
    ...
    width: 100%;}
           ...
           <br>
            <p>Доводим до Вашего сведения, Согласно договорам  заявки №<?php echo $row['number'];?> на транспортно-экспедиционные услуги от <?php echo $date;?> заключенной между FAHRWEST LOGISTIC SRL(Заказчик) и <?php echo $row11['name'].' '.$row11['company_form']; ?> (Перевозчик), автомобиль с регистрационными номером <?php echo $row['carNumber'];?>, по направлению <?php echo $row["from"]?> - <?php echo $row["to"]?></p>
      
            <p>В результате переговоров с директором компании, мы узнаём, что компания <?php echo $row11['name'].' '.$row11['company_form']; ?> в связи с возникшей проблемой с документами на автомобиль не может забрать груз, что за собой влечёт срыв погрузки в одностороннем порядке.</p>
            <p>В Связи с этим согласно пункту, номер 26 в договоре-заявке №<?php echo $row['number']?> от <?php echo $date ?>:</p>
            <p>За отказ от погрузки(разгрузки) по вине заказчика – перевозчика, виновный выплачивает штраф в размере 20% от суммы договора
            </p>
            <br>
            <p>Стоимость перевозки согласно договору-заявке №<?php echo $row['number']?> от <?php echo $date ?> = <?php echo $row['carrierPrice'] ?> <?php echo $row['currency'] ?> * 20% = <?php echo $row['customerPrice']*0.2 ?> <?php echo $row['currency'] ?></p>
            <p>Срыв погрузки</p>
            <p>Итого общая сумма задолженность компании <?php echo $row11['name'].' '.$row11['company_form']; ?> перед компанией ITECO CORP SRL - <?php echo $row['customerPrice']*0.2 ?> <?php echo $row['currency'] ?></p>
            <p>Просим Вас перечислить на наш расчётный счёт сумму в размере <?php echo $row['customerPrice']*0.2 ?> <?php echo $row['currency'] ?> согласно договору-заявке №<?php echo $row['number']?> от <?php echo $data ?></p>
            <br>
            <p>FAHRWEST LOGISTIC SRL</p>
            <p>Bd.Renasterii Nationale 6, ap.90</p>  
            <p>ф\к 1022600012316</p>
            <p>ндс 0612266</p>
            <p>IBAN MD66AG000000022514703441 AGRNMD2X441</p> 
