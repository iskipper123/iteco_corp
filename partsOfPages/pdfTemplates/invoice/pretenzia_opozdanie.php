...
<html lang="ru">
    ...
    width: 100%;}
           ...
           <br>
            <p>Доводим до Вашего сведения, Согласно договорам  заявки №<?php echo $row['number'];?> на транспортно-экспедиционные услуги от <?php echo $date;?> заключенной между FAHRWEST LOGISTIC SRL(Заказчик) и <?php echo $row12['name'].' '.$row12['company_form']; ?> (Перевозчик). На погрузку был поставлен автомобиль с регистрационными номером <?php echo $row['carNumber'];?>, по направлению <?php echo $row["from"]?> - <?php echo $row["to"]?>. Согласно условиям, указанным в договор-заявке №<?php echo $row['number'];?> пункт №5 За каждые сутки опоздания доставки груза, перевозчик выплачивает штраф в размере 50$ если грузополучатель на территории РМ и СНГ и 50€ если получатель груза в странах ЕС. </p>
      
            <br>
            <p>За нарушения условий в договор-заявки, просим Вас выплатить штраф в размере ____€(____ евро), в течении 5 дней. В случае задержки сроков оплаты Перевозчик обязуется оплатить пеню в размере 4% от суммы претензии за каждый день просрочки.</p>
            <br><br><br>
            <p>М.П.</p>
            </textarea>
        </div>
